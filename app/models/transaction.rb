class Transaction < ActiveRecord::Base
  belongs_to :user
  belongs_to :tranable, polymorphic: true
  ...
  validates :amount, numericality: { greater_than_or_equal_to: 0 },
                    presence: true
  validates_presence_of :currency, :description, :tran_date
  ...
  enum currency: { naira: 0, us_dollar: 1, gb_pound: 2 }

  scope :most_recent, -> { order("tran_date DESC").limit(5) }
